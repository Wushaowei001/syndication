...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        CLLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
			...
			if (SQLITE_LOCKED == rc) {
				...
				if (rc != SQLITE_LOCKED) {
					CLLog(@"Unexpected result from sqlite3_reset (%d) rs", rc);
				}
			}
            usleep(20);
            ...
            if ([parentDB busyRetryTimeout] && (numberOfRetries++ > [parentDB busyRetryTimeout])) {
                ...
                CLLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [parentDB databasePath]);
                CLLog(@"Database busy");
                break;
            }
        }
            ...
            CLLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            CLLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    CLLog(@"Warning: I could not find the column named '%@'.", columnName);
